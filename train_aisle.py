...
from tf_util1 import lstm_layer, time_distributed_dense_layer, sequence_log_loss
from tf_base1 import TFBaseModel
...
class DataReader(object):
    ...
    def train_batch_generator(self, b_size):
    ...
    def val_batch_generator(self, b_size):
        return self.batch_generator(
            b_size=b_size,
            df=self.val_df,
            shuff=True,
    ...
    def test_batch_generator(self, b_size):
        return self.batch_generator(
            b_size=b_size,
            df=self.test_df,
            shuff=False,
    ...
    def batch_generator(self, b_size, df, shuff=True, n_epoch=10000, is_test=False):
        batch_gen = df.batch_generator(b_size, shuff=shuff, n_epoch=n_epoch, allow_smaller_final_batch=is_test)
        for batch in batch_gen:
            ...
            batch['next_is_ordered'] = np.roll(batch['order_history'], -1, axis=1)
            if not is_test:
...
class rnn(TFBaseModel):
    ...
    def get_input_sequences(self):
        ...
        self.order_history = tf.placeholder(tf.int32, [None, 100])
        ...
        order_history = tf.one_hot(self.order_history, 2)
