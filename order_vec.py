...
if __name__ == '__main__':
    ...
    for idx, row in user_data.iterrows():
        ...
        order_sizes = [len(list(i)) for i in orders]
        ...
        order_number_history[idx, :], _ = pad_1d(list(map(float, row['order_numbers'].split())), 100)
        order_dow_history[idx, :], _ = pad_1d(list(map(int, row['order_dows'].split())), 100)
        order_hour_history[idx, :], _ = pad_1d(list(map(int, row['order_hours'].split())), 100)
        days_since_prior_order_history[idx, :], _ = pad_1d(list(map(int, row['days_since_prior_orders'].split())), 100)
